 ...
 */
function addAliasToAClass(
  thisAlias = "d:new alias 1",
  courseId = "526390027173"
) {
  let alias = {
    alias: thisAlias,
  };

  let response3 = Classroom.Courses.Aliases.create(
    (resource = alias),
    (courseId = courseId)
  );
...
function getStudentData(courseId = "595042513070") {
  let studentData = {};

  studentData.courseId = courseId;
  studentData.name = getCourseData(courseId);
  studentData.studentList = []

  let response = Classroom.Courses.Students.list(courseId);
 
  if (Object.keys(response).length === 0){
    studentData.studentList = []
    
  } else {
    studentData.studentList = response.students.map(x => x.profile.emailAddress);
  }
