...
function getCourseById(id = "526390027173") {
  ...
  courseData.teachers = courseData.teachers.filter(
    x => x !== courseData.courseTeacher
  );
 ...
 */
function addCoteacherToClass(
  classroomId = "526390027173",
  teacherId = "craig.davis@mvsdschools.org"
) {
  try {
    let response = Classroom.Courses.Teachers.create(
      { userId: teacherId },
      classroomId
    );
  } catch (err) {
...
function addStudentToClass(
  classroomId = "526390027173",
  studentId = "craig.davis@mvsdschools.org"
) {
  try {
    let response = Classroom.Courses.Students.create(
      {
        userId: studentId,
      },
      classroomId
...
function createCourse(data) {
  ...
  let provisionedSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Provisioned");
